...
@section('title', 'Expandor — CRM de campo')
...
@section('content')
    ...
    <section class="hero">
        ...
        <p>CRM de campo para equipes que vendem na rua — mapa, visitas, follow-ups e comissões em um só lugar.</p>
        <div class="cta-row">
            <a class="btn btn-amber" href="{{ route('marketplace.plans') }}">Ver planos</a>
            <a class="btn btn-ghost" href="{{ route('signup.create') }}">Teste grátis</a>
    ...
    <section>
        ...
        <p class="muted" style="margin-top:0; margin-bottom:1.25rem;">Assine online com PIX ou cartão. Provisionamento automático após o pagamento.</p>
        ...
        <div class="grid">
            @forelse($featuredPlans as $plan)
                <div class="card plan-card">
                    ...
                    @if((float) $plan->price > 0)
                        <a class="btn" href="{{ route('marketplace.subscribe', ['plan_id' => $plan->id]) }}">Assinar</a>
                    @else
                        <a class="btn btn-ghost" href="{{ route('signup.create') }}">Começar teste</a>
                    @endif
                </div>
            @empty
